...
SWEP.WorldModel					= 'models/didvaitel/rust_hammer.mdl'
...
SWEP.VElements = {
	["hammer"] = { type = "Model", model = "models/didvaitel/rust_hammer.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector( 4, 1.5, -6.2), angle = Angle( -82, 0, 0 ), size = Vector( 1, 1, 1 ), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 1, bodygroup = {} }
}

SWEP.WElements = {
	["hammer"] = { type = "Model", model = "models/didvaitel/rust_hammer.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector( 4, 2, -2.5 ), angle = Angle( -70, 0, 90 ), size = Vector( 1, 1, 1 ), color = Color( 255, 255, 255, 255 ), surpresslightning = false, material = "", skin = 1, bodygroup = {} },
}
...
if SERVER then
	if IsValid(tr.Entity) then
		if (tr.Entity:GetClass() == "prop_dynamic" or tr.Entity:GetClass() == "func_door" or tr.Entity:GetClass() == "prop_door_rotating") then
			if tr.Entity.breaked == nil then return end
			if tr.Entity.breaked == false then return end
			local ydacha = math.random(0,100)
			if ydacha < 80 then return DarkRP.notify(self.Owner, 1,4, "Дверь осталась сломанной!") end
			for k,v in pairs(ents.FindInSphere(tr.Entity:GetPos(),80)) do
				v.breaked = false
			end
			local xpm =  math.random( 500, 1500 )
			local nagrada =  math.random( 500, 3000 )
			self.Owner:addXP(xpm, true)
			self.Owner:addMoney(nagrada)
			DarkRP.notify(self.Owner, 3,4, "Вы успешно починили дверь! получено ".. xpm.."XP и "..nagrada.."$")
		end
	end
end
